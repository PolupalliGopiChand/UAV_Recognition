...
%% ============================ BLOCK 5 ===================================
[feat_uav, feat_names] = B5_feature_extractor(TF_sig_uav);
[feat_bird, ~] = B5_feature_extractor(TF_sig_bird);

fprintf('\nBlock-5 Output (Feature Vector Summary):\n');
T_feat = array2table([feat_uav(:)'; feat_bird(:)'], 'VariableNames', feat_names, ...
    'RowNames', {'UAV','Bird'});
disp(T_feat);
...
%% -------- B5 --------
function [feat_vec, feat_names] = B5_feature_extractor(TF_sig)

fd = TF_sig.fd(:);
t_stft = TF_sig.t_stft(:);
S_norm = TF_sig.S_normalized;
dop_marg = TF_sig.doppler_marginal(:);
tf_ridge = TF_sig.tf_ridge(:);

eps_val = 1e-9;

% Time/Frequency resolution (for normalization)
df = mean(abs(diff(fd)));
dt = mean(abs(diff(t_stft)));

% Doppler marginal statistics
mu_dop = sum(fd .* dop_marg) / (sum(dop_marg) + eps_val);
var_dop = sum(((fd - mu_dop).^2) .* dop_marg) / (sum(dop_marg) + eps_val);
std_dop = sqrt(var_dop + eps_val);
skew_dop = sum(((fd - mu_dop).^3) .* dop_marg) / ((std_dop^3) * (sum(dop_marg) + eps_val));
kurt_dop = sum(((fd - mu_dop).^4) .* dop_marg) / ((std_dop^4) * (sum(dop_marg) + eps_val));

% Ridge statistics
ridge_mean = mean(tf_ridge);
ridge_std = std(tf_ridge);
ridge_range = max(tf_ridge) - min(tf_ridge);
ridge_slope = mean(abs(diff(tf_ridge))) / (dt + eps_val);

% Concentration/flatness measures
spec_entropy = TF_sig.spectral_entropy;
spec_entropy_norm = spec_entropy / log(length(fd) + eps_val);
energy_centroid = mu_dop;
energy_spread = std_dop;

% Energy ratios around body band
fc = 0.05 * max(abs(fd));
body_band = abs(fd) < fc;
harm_band = abs(fd) >= fc;
body_energy = sum(dop_marg(body_band)) * df;
harm_energy = sum(dop_marg(harm_band)) * df;
harm_body_ratio = harm_energy / (body_energy + eps_val);
